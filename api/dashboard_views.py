...
from payments.services import create_payment_for_quote, initiate_stk_push, normalize_mpesa_phone
        ...
        recent_jobs = [
            {
                ...
                "quote_request_id": job.source_quote_request_id,
                "quote_id": job.source_quote_id,
...
class PartnerDashboardHomeView(BaseDashboardHomeView):
    ...
    def get(self, request):
        jobs = ManagedJob.objects.filter(_partner_managed_job_filter(request.user)).select_related(
            "client",
            "assigned_shop",
...
class DashboardCountsView(APIView):
    ...
    def _manager_counts(self, user) -> dict[str, int]:
        ...
        jobs = ManagedJob.objects.filter(_partner_managed_job_filter(user)).distinct()
        return {
...
def _dashboard_dict(value) -> dict:
    return value if isinstance(value, dict) else {}


def _dashboard_first_value(*values):
    for value in values:
        if value is None:
            continue
...
def _quote_product_label(quote_request: QuoteRequest, latest_response: Quote | None = None) -> str:
    root = _dashboard_dict(getattr(quote_request, "request_snapshot", None))
    nested = _dashboard_dict(root.get("request_snapshot"))
    calculator_inputs = _dashboard_dict(root.get("calculator_inputs"))
    response_snapshot = _dashboard_dict(getattr(latest_response, "response_snapshot", None))
    response_request = _dashboard_dict(response_snapshot.get("request_snapshot"))
    response_inputs = _dashboard_dict(response_snapshot.get("calculator_inputs"))
    draft_inputs = _dashboard_dict(getattr(getattr(quote_request, "source_draft", None), "calculator_inputs_snapshot", None))
    item = quote_request.items.select_related("product").order_by("id").first()
    product = getattr(item, "product", None) if item is not None else None
    label = _dashboard_first_value(
        root.get("product_label"),
        root.get("product_type"),
        nested.get("product_label"),
        nested.get("product_type"),
        calculator_inputs.get("product_label"),
        calculator_inputs.get("product_type"),
        response_snapshot.get("product_label"),
        response_snapshot.get("product_type"),
        response_request.get("product_label"),
        response_request.get("product_type"),
        response_inputs.get("product_label"),
        response_inputs.get("product_type"),
        draft_inputs.get("product_label"),
        draft_inputs.get("product_type"),
        getattr(item, "title", None),
        getattr(product, "name", None),
        getattr(quote_request, "notes", None),
    )
    return str(label or "Print request")


def _quote_partner_pricing_payload(quote_request: QuoteRequest, latest_response: Quote | None) -> dict[str, object]:
    split = getattr(latest_response, "financial_split", None) if latest_response is not None else None
    response_snapshot = _dashboard_dict(getattr(latest_response, "response_snapshot", None))
    customer_pricing = _dashboard_dict(response_snapshot.get("customer_pricing"))
    internal_pricing = _dashboard_dict(response_snapshot.get("internal_pricing_snapshot"))
    pricing = _dashboard_dict(response_snapshot.get("pricing"))
    totals = _dashboard_dict(response_snapshot.get("totals"))
    managed_job = quote_request.managed_jobs.order_by("-id").first()

    client_total = _dashboard_first_value(
        getattr(split, "client_total", None),
        customer_pricing.get("final_client_price"),
        customer_pricing.get("estimated_total"),
        internal_pricing.get("final_client_price"),
        pricing.get("grand_total"),
        totals.get("grand_total"),
        getattr(latest_response, "total", None),
        getattr(managed_job, "client_total", None),
    )
    production_cost = _dashboard_first_value(
        getattr(split, "production_cost", None),
        internal_pricing.get("production_cost"),
        customer_pricing.get("production_cost"),
        getattr(managed_job, "production_total", None),
    )
    gross_margin = _dashboard_first_value(
        getattr(split, "gross_margin", None),
        internal_pricing.get("gross_margin"),
        internal_pricing.get("broker_margin_amount"),
        customer_pricing.get("gross_margin"),
        customer_pricing.get("broker_margin_amount"),
    )

    margin_percent = None
    try:
        production_decimal = Decimal(str(production_cost))
        margin_decimal = Decimal(str(gross_margin))
        if production_decimal > 0:
            margin_percent = (margin_decimal / production_decimal * Decimal("100")).quantize(Decimal("0.01"))
    except Exception:
        margin_percent = _dashboard_first_value(
            internal_pricing.get("gross_margin_percent"),
            customer_pricing.get("gross_margin_percent"),
        )
    ...
    return {
        ...
        "gross_margin": str(gross_margin) if gross_margin is not None else None,
        "manager_margin": str(gross_margin) if gross_margin is not None else None,
...
class BaseRoleDetailView(BaseDashboardHomeView):
    ...
    def _quote_row(self, quote_request: QuoteRequest, *, request=None) -> dict[str, object]:
        ...
        row = {
            ...
            "product": _quote_product_label(quote_request, latest_response),
            ...
            **_quote_partner_pricing_payload(quote_request, latest_response),
        }
        managed_job = quote_request.managed_jobs.order_by("-id").first()
        row["managed_job"] = {
            "id": managed_job.id,
...
class PartnerQuoteSendToClientView(ManagerCapablePartnerQuoteView):
    ...
    def post(self, request, pk):
        ...
        if is_offline_quote:
            ...
            if payment_phone:
                try:
                    payment_phone = normalize_mpesa_phone(payment_phone)
...
class PartnerJobListDetailView(BaseRoleDetailView):
    ...
    def get_queryset(self, request):
        return ManagedJob.objects.filter(_partner_managed_job_filter(request.user)).select_related("client", "assigned_shop").order_by("-updated_at", "-created_at")
