...
def api_exception_handler(exc, context):
    ...
    if isinstance(data, dict):
        ...
        app_code = _first_scalar(data.get("code"))
        normalized = {
            "code": app_code or ("VALIDATION_ERROR" if response.status_code == 400 else _status_to_code(response.status_code)),
        ...
        resend_available = _first_scalar(data.get("resend_available"))
        if resend_available is not None:
            normalized["resend_available"] = resend_available in (True, "true", "True", "1", 1)
