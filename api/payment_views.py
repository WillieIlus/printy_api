...
def _serialize_managed_job_settlement(managed_job: ManagedJob, user) -> dict:
    payload = {
        ...
        "status": _settlement_status(managed_job),
    }
    role = get_actor_role(user)
    if _is_managed_job_broker(user, managed_job):
        return {
            **payload,
            ...
            "broker_payout": str(managed_job.broker_payout) if managed_job.broker_payout is not None else None,
            "printy_fee": str(managed_job.printy_fee) if managed_job.printy_fee is not None else None,
            "production_cost": str(getattr(managed_job, "production_cost", None)) if getattr(managed_job, "production_cost", None) is not None else None,
        }
    if role == ActorRole.SHOP:
        assignment = managed_job.assignments.filter(reassigned_from__isnull=True).first()
        shop_payout = getattr(assignment, "shop_payout", None)
        return {
            **payload,
