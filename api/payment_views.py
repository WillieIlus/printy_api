...
from payments.services import handle_stk_callback, initiate_stk_push, normalize_mpesa_phone
...
class PaymentInitiateSTKView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.warning("Quote M-Pesa STK push failed payment_id=%s quote_id=%s user_id=%s: %s", getattr(payment, "id", None), quote_id, getattr(request.user, "id", None), exc)
...
class ManagedJobPaymentInitiateSTKView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            logger.warning("Managed job payment STK push provider failed managed_job_id=%s user_id=%s: %s", pk, getattr(request.user, "id", None), exc)
            return Response({"detail": str(exc)}, status=status.HTTP_502_BAD_GATEWAY)
        return Response(
            {
                "payment_id": payment.id,
...
class ManagedJobMpesaSTKPushView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            logger.warning("Managed job STK push provider failed managed_job_id=%s user_id=%s: %s", pk, getattr(request.user, "id", None), exc)
            return Response({"detail": str(exc)}, status=status.HTTP_502_BAD_GATEWAY)
        except Exception:
