...
def _can_access_quote_request_attachment(*, user, quote_request: QuoteRequest) -> bool:
    if not user or not user.is_authenticated:
        return False
    if user.is_staff:
        return True
    if quote_request.created_by_id == user.id or quote_request.on_behalf_of_id == user.id:
        return True
    if quote_request.assigned_manager_id == user.id:
        return True
    if quote_request.shop_id and quote_request.shop.owner_id == user.id:
        return True
    if quote_request.managed_jobs.filter(broker=user).exists():
        return True
    return False


class QuoteRequestAttachmentDownloadView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, pk):
        attachment = get_object_or_404(
            QuoteRequestAttachment.objects.select_related(
                "quote_request",
                "quote_request__shop",
        ...
        if not _can_access_quote_request_attachment(user=request.user, quote_request=attachment.quote_request):
            return Response({"detail": "Not authorized."}, status=status.HTTP_403_FORBIDDEN)
        if not attachment.file:
            return Response({"detail": "File is not available for download."}, status=status.HTTP_404_NOT_FOUND)
        return FileResponse(
