...
from quotes.models import EmailOutbox, Quote, QuoteRequest
...
class PartnerQuoteSendToClientTestCase(TestCase):
    ...
    def test_send_to_client_supports_canonical_quantity_pricing(self):
        ...
        self.assertEqual(outbox.recipient_email, self.end_client.email)
        self.assertEqual(outbox.quote, quote)
        self.assertIn("quote", outbox.subject.lower())


    @patch("api.dashboard_views.initiate_stk_push")
    def test_offline_send_to_client_normalizes_mpesa_phone_before_payment_and_stk(self, mock_initiate_stk_push):
        mock_initiate_stk_push.return_value = SimpleNamespace(id=321, status="pending")
        quote_request = QuoteRequest.objects.create(
            shop=self.shop,
            created_by=self.partner,
            on_behalf_of=None,
            customer_name="Offline Client",
            customer_phone="0700 000 000",
            status=QuoteStatus.DRAFT,
            request_snapshot={
                "source": "partner_quote_builder",
                "pending_client": {"name": "Offline Client", "phone": "0700 000 000"},
            },
        )
        quote = Quote.objects.create(
            quote_request=quote_request,
        ...
        self.assertEqual(response.status_code, 200)
        payment = Payment.objects.get(quote=quote)
