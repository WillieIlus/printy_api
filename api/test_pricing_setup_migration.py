...
class PricingSetupMigrationTests(TestCase):
    ...
    def test_same_finishing_slug_can_exist_for_different_shops(self):
        other_shop = Shop.objects.create(
            owner=self.other,
            name="Other Finishing Shop",
            slug="other-finishing-shop",
            is_active=True,
        )

        first = FinishingRate.objects.create(
            shop=self.shop,
            name="Cutting Standard",
            slug="cutting",
        ...
        second = FinishingRate.objects.create(
            shop=other_shop,
            name="Cutting Standard",
            slug="cutting",
            charge_unit=ChargeUnit.FLAT,
        ...
        self.assertEqual(FinishingRate.objects.filter(slug="cutting").count(), 2)

    def test_legitimate_finishing_billing_variants_can_coexist(self):
        flat = FinishingRate.objects.create(
            shop=self.shop,
            name="Cutting Standard",
            slug="cutting",
            charge_unit=ChargeUnit.FLAT,
        ...
        per_piece = FinishingRate(
            shop=self.shop,
            name="Cutting Standard Per Piece",
            slug="cutting",
        ...
        per_piece.full_clean()
        per_piece.save()
        self.assertEqual(FinishingRate.objects.filter(shop=self.shop, slug="cutting").count(), 2)
        self.assertNotEqual(flat.billing_basis, per_piece.billing_basis)

    def test_identical_finishing_rate_identity_cannot_recur(self):
        FinishingRate.objects.create(
            shop=self.shop,
            name="Cutting Standard",
            slug="cutting",
        ...
        duplicate = FinishingRate(
            shop=self.shop,
            name="Cutting Duplicate",
            slug="cutting",
            charge_unit=ChargeUnit.FLAT,
            billing_basis=FinishingBillingBasis.FLAT_PER_JOB,
            side_mode=FinishingSideMode.IGNORE_SIDES,
    ...
    def test_repeating_rate_card_setup_patch_is_idempotent_and_updates_finishing_rates(self):
        self.client.force_authenticate(user=self.owner)
        payload = {
            "paper_rows": [],
        ...
        first = self.client.patch(f"/api/shops/rate-card/setup/?shop_slug={self.shop.slug}", payload, format="json")
        second = self.client.patch(
            f"/api/shops/rate-card/setup/?shop_slug={self.shop.slug}",
            {"paper_rows": [], "finishing_rows": [{"key": "cutting", "price": "520.00", "active": True}]},
            format="json",
        )

        self.assertEqual(first.status_code, 200)
        self.assertEqual(second.status_code, 200)
        rates = FinishingRate.objects.filter(shop=self.shop, slug="cutting")
        self.assertEqual(rates.count(), 1)
        self.assertEqual(rates.get().price, Decimal("520.00"))

    def test_amazing_ace_setup_patch_succeeds_and_finishing_persists_after_get(self):
        amazing_shop = Shop.objects.create(
            owner=self.owner,
            name="Amazing Ace",
            slug="amazingace",
            is_active=True,
        ...
        payload = {
            ...
            "finishing_rows": [
                ...
                {"key": "cutting", "price": "150.00", "active": True},
            ],
        }

        patch_response = self.client.patch("/api/shops/rate-card/setup/?shop_slug=amazingace", payload, format="json")
        get_response = self.client.get("/api/shops/rate-card/setup/", {"shop_slug": amazing_shop.slug})

        self.assertEqual(patch_response.status_code, 200)
        self.assertEqual(get_response.status_code, 200)
        patch_rows = {row["key"]: row for row in patch_response.json()["finishing_rows"]}
        get_rows = {row["key"]: row for row in get_response.json()["finishing_rows"]}
        self.assertTrue(patch_rows["cutting"]["active"])
        self.assertEqual(patch_rows["cutting"]["price"], "150.00")
        self.assertTrue(get_rows["matte_lamination_double"]["active"])
        self.assertEqual(get_rows["matte_lamination_double"]["minimum_charge"], "60.00")

    def test_finishing_save_failure_rolls_back_rate_card_setup(self):
        with patch("services.pricing.mvp_rate_card._persist_finishing_rows", side_effect=RuntimeError("finishing failed")):
            with self.assertRaises(RuntimeError):
                save_shop_rate_card_setup(
                    ...
                    finishing_rows=[{"key": "cutting", "price": "480.00", "active": True}],
                    shop_details={"shop_name": "Rolled Back Shop"},
                    completed=False,
                )

        self.shop.refresh_from_db()
        self.assertEqual(self.shop.name, "Canonical Pricing Shop")
        self.assertFalse(Paper.objects.filter(shop=self.shop, gsm=300).exists())
