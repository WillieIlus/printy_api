...
class PublicShopQuotePreviewTestCase(TestCase):
    def setUp(self):
        ...
        self.manager = User.objects.create_user(
            email="direct-preview-manager@test.com",
            password="pass12345",
    ...
    def test_manager_gets_direct_shop_priced_preview_and_scoped_draft(self):
        ...
        self.assertEqual(draft.user_id, self.manager.id)
    ...
    def test_anonymous_buyer_denied_direct_shop_preview_endpoints(self):
        response = self.client.post(
            "/api/public/shops/direct-preview-shop/quote-preview/",
            self._payload(),
            format="json",
        ...
        self.assertIn(calculator_response.status_code, (401, 403))

        rate_card_response = self.client.get(
            "/api/shops/direct-preview-shop/rate-card/"
        )
        self.assertIn(rate_card_response.status_code, (401, 403))
    ...
    def test_manager_anonymized_shop_preview_still_hides_shop_identity(self):
        self.client.force_authenticate(user=self.manager)
        response = self.client.post(
            "/api/public/shops/direct-preview-shop/calculator-preview/",
            self._payload(),
