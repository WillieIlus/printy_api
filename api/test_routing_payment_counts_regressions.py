...
from jobs.assignment_services import accept_assignment, mark_assignment_in_production
...
class AdminPaymentConfirmationRegressionTestCase(TestCase):
    ...
    def test_manager_and_assigned_shop_see_in_production_without_cross_shop_visibility(self):
        other_owner = User.objects.create_user(email="other-shop@example.com", password="pass", role=User.Role.PRODUCTION)
        other_shop = Shop.objects.create(name="Other Shop", slug="other-shop", owner=other_owner, is_active=True)
        managed_job = ManagedJob.objects.create(
            title="Production visibility job",
        ...
        accept_assignment(assignment=assignment, actor=self.shop_owner)
        assignment.refresh_from_db()
        mark_assignment_in_production(assignment=assignment, actor=self.shop_owner)
        managed_job.refresh_from_db()

        self.assertEqual(managed_job.status, ManagedJobStatus.IN_PRODUCTION)
        self.api_client.force_authenticate(user=self.manager)
        partner_response = self.api_client.get("/api/dashboard/partner/jobs/")
        self.assertEqual(partner_response.status_code, 200)
        partner_rows = {row["id"]: row for row in partner_response.json()["results"]}
        self.assertEqual(partner_rows[managed_job.id]["status"], ManagedJobStatus.IN_PRODUCTION)
        self.assertEqual(partner_rows[managed_job.id]["assigned_shop_name"], self.shop.name)

        self.api_client.force_authenticate(user=self.shop_owner)
        production_response = self.api_client.get("/api/dashboard/production/jobs/")
        self.assertEqual(production_response.status_code, 200)
        production_rows = {row["id"]: row for row in production_response.json()["results"]}
        self.assertEqual(production_rows[managed_job.id]["status"], ManagedJobStatus.IN_PRODUCTION)
        self.assertEqual(production_rows[managed_job.id]["assigned_shop_name"], self.shop.name)

        self.api_client.force_authenticate(user=other_owner)
        other_response = self.api_client.get("/api/dashboard/production/jobs/")
        self.assertEqual(other_response.status_code, 200)
        self.assertNotIn(managed_job.id, {row["id"] for row in other_response.json()["results"]})
        self.assertFalse(ManagedJob.objects.filter(assigned_shop=other_shop, pk=managed_job.id).exists())

    def test_manager_sees_in_production_job_owned_by_assigned_request_when_broker_is_missing(self):
        ...
        quote_request = QuoteRequest.objects.create(
            created_by=self.client_user,
            assigned_manager=self.manager,
            customer_name="Brokerless Job Client",
            customer_email="brokerless-job@example.com",
        )
        ...
        managed_job = ManagedJob.objects.create(
            title="Brokerless production visibility job",
            source_quote_request=quote_request,
            source_quote=quote,
            client=self.client_user,
            broker=None,
            assigned_shop=self.shop,
            created_by=self.client_user,
            status=ManagedJobStatus.IN_PRODUCTION,
            payment_status=ManagedJobPaymentStatus.CONFIRMED,
            assignment_status="assigned",
        )
        ...
        counts_response = self.api_client.get("/api/dashboard/counts/")
        self.assertEqual(counts_response.status_code, 200)
        self.assertGreaterEqual(counts_response.json()["counts"]["active_jobs"], 1)

        self.api_client.force_authenticate(user=other_manager)
        other_response = self.api_client.get("/api/dashboard/partner/jobs/")
        self.assertEqual(other_response.status_code, 200)
        self.assertNotIn(managed_job.id, {row["id"] for row in other_response.json()["results"]})
...
@override_settings(ACCOUNT_EMAIL_VERIFICATION="mandatory", PASSWORD_HASHERS=["django.contrib.auth.hashers.MD5PasswordHasher"])
class ProductionLoginRegressionTestCase(TestCase):
    def setUp(self):
        self.api_client = APIClient()

    def test_verified_production_user_can_login_through_browser_login_alias(self):
        user = User.objects.create_user(
            email="production-login-alias@test.com",
            password="Pass12345",
            name="Production Login Alias",
