...
class SEOAPITestCase(TestCase):
    ...
    def test_seo_routes_returns_canonical_urls(self):
        ...
        self.assertNotIn("/shops/test-shop", locs)
        for d in data:
            self.assertIn("loc", d)
...
class PublicShopsAPITestCase(TestCase):
    """Test public shop and catalog endpoints (restricted to printing managers/shop staff)."""

    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            email="s@t.com", password="pass", role=User.Role.PRODUCTION
        )
        self.client.force_authenticate(user=self.user)
        self.shop = Shop.objects.create(
    ...
    def test_anonymous_buyer_denied_shop_directory(self):
        r = APIClient().get("/api/public/shops/")
...
class ShopsNearbyAPITestCase(TestCase):
    """Test GET /api/shops/nearby/ — bounding box geo search (restricted to printing managers/shop staff)."""
...
class ShopRateCardPaperDuplexSurchargeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.owner = User.objects.create_user(
            email="ratecard@test.com", password="pass12345", role=User.Role.PRODUCTION
        )
        self.client.force_authenticate(user=self.owner)
...
@override_settings(
    ...
    DEFAULT_FROM_EMAIL="Printy <hello.printy@gmail.com>",
)
