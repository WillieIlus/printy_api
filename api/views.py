...
from api.permissions import PrintManagerMarketplaceAccess
...
from quotes.models import QuoteItem, QuoteItemAttachment, QuoteItemFinishing, QuoteRequest, QuoteShareLink, Quote
...
from quotes.draft_files import ensure_calculator_draft_file, ensure_calculator_draft_file_for_request
...
from .serializers import (
    ...
    QuoteItemAttachmentSerializer,
    QuoteItemAttachmentUploadSerializer,
...
class PublicShopViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    Restricted to printing managers and shop staff: buyers only ever deal with
    printing managers, never with printers or their shops.
    """

    permission_classes = [PrintManagerMarketplaceAccess]
...
class ShopRateCardView(APIView):
    """
    GET /api/shops/{slug}/rate-card/ — rate card for a shop.
    Returns printing rates, paper prices, and finishing services for a printing
    manager (or shop staff) to build a marked-up rate card. Buyers never see
    raw printer rates.
    """
...
class ShopRateCardForCalculatorView(APIView):
    ...
    Returns templates, papers, printing_rates, finishing_rates, materials for the
    calculator. Restricted to printing managers and shop staff.
    """
...
class ShopsNearbyView(APIView):
    ...
    Restricted to printing managers and shop staff.
    """
...
class QuoteRequestViewSet(viewsets.ModelViewSet):
    ...
    def price(self, request, pk=None):
        """Seller: calculate & lock prices, create Quote, set QuoteRequest -> quoted."""
    ...
    def send(self, request, pk=None):
        """Seller: ensure Quote sent (no-op if already quoted)."""
        qr = self.get_object()
        if qr.shop.owner_id != request.user.id:
            return Response({"detail": "Not your shop."}, status=status.HTTP_403_FORBIDDEN)
...
class CalculatorDraftViewSet(viewsets.ViewSet):
    """
    ...
    def active(self, request):
        return Response(
            {"detail": "Shop-scoped quote drafts are disabled. Use /api/calculator/drafts/ for client drafts."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    def request_quote(self, request, pk=None):
        return Response(
            {"detail": "Client quote-draft shop routing is disabled. Use /api/intake/submit/ for client quote requests."},
            status=status.HTTP_400_BAD_REQUEST,
        )
        """POST /api/quote-drafts/{id}/request-quote/ — submit draft (status -> SUBMITTED)."""
        draft = get_object_or_404(QuoteRequest.objects.select_related("shop"), pk=pk)
    ...
    def download_pdf(self, request, pk=None):
        ...
        if draft.created_by_id != request.user.id:
            return Response({"detail": "Not your quote request."}, status=status.HTTP_403_FORBIDDEN)
        try:
            pdf_bytes = render_calculator_draft_pdf(draft)
        except ImportError:
...
class QuoteSharePublicView(APIView):
    ...
    def get(self, request, token):
        ...
        link = get_object_or_404(
            QuoteShareLink.objects.select_related("quote__shop", "quote__quote_request"),
            token=token,
        )
        ...
        quote = link.quote
        quote = Quote.objects.filter(pk=quote.pk).select_related(
            "shop", "quote_request"
        ).prefetch_related(
            "items__product",
            "items__paper",
...
class QuoteViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        draft_file = ensure_calculator_draft_file_for_request(user=self.request.user, draft=quote_request)
    ...
    def share(self, request, pk=None):
        ...
        Uses or creates Quote for the share link. Optional body: { expires_at: "2025-12-31T23:59:59Z" }.
        """
        import secrets
        ...
        # Get or create Quote for sharing
        quote = quote_request.get_latest_quote()
        if not quote:
            from quotes.pricing_service import compute_and_store_pricing
            from django.db import transaction
            ...
            with transaction.atomic():
                ...
                quote_request.items.update(quote=quote)
    ...
    def send(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            quote = quote.get_latest_quote()
            if not quote:
                rev = quote.quotes.count() + 1
                quote = Quote.objects.create(
                    quote_request=quote,
                    shop=shop,
                    created_by=request.user,
                ...
                quote.items.update(quote=quote)
            else:
                ...
                quote.sent_at = now
                quote.save(update_fields=["whatsapp_message", "sent_at", "updated_at"])
...
class CalculatorDraftItemViewSet(viewsets.ModelViewSet):
    """
    ...
    def get_queryset(self):
        quote_pk = self.kwargs.get("calculator_draft_pk")
        return QuoteItem.objects.filter(quote_request_id=quote_pk).select_related("product")

    def get_quote_request(self):
        return get_object_or_404(QuoteRequest, pk=self.kwargs["calculator_draft_pk"])
...
class CalculatorDraftItemRequestQuoteView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request, calculator_draft_pk, pk):
        return Response(
            {"detail": "Client quote-draft item shop routing is disabled. Use /api/intake/submit/ for client quote requests."},
            status=status.HTTP_400_BAD_REQUEST,
        )
        draft = get_object_or_404(QuoteRequest.objects.select_related("shop"), pk=calculator_draft_pk)
...
class CalculatorDraftItemAttachmentViewSet(viewsets.ModelViewSet):
    """
    GET/POST /api/quote-drafts/{draft_id}/items/{item_id}/attachments/
    DELETE /api/quote-drafts/{draft_id}/items/{item_id}/attachments/{pk}/
    """
    ...
    def get_quote_item(self):
        return get_object_or_404(
            QuoteItem.objects.select_related("quote_request"),
            pk=self.kwargs["quote_item_pk"],
            quote_request_id=self.kwargs["calculator_draft_pk"],
        )

    def _check_access(self):
        qr = self.get_quote_request()
        item = self.get_quote_item()
        user = self.request.user
        can_read = user.is_staff or qr.created_by_id == user.id
        can_write = can_read and qr.status == QuoteStatus.DRAFT
        return qr, item, can_read, can_write

    def get_queryset(self):
        _, item, can_read, _ = self._check_access()
        if not can_read:
            return QuoteItemAttachment.objects.none()
        return QuoteItemAttachment.objects.filter(quote_item=item)

    def get_serializer_class(self):
        if self.action == "create":
            return QuoteItemAttachmentUploadSerializer
        return QuoteItemAttachmentSerializer

    def list(self, request, *args, **kwargs):
        _, _, can_read, _ = self._check_access()
        if not can_read:
            return Response({"detail": "Not authorized."}, status=status.HTTP_403_FORBIDDEN)
        return super().list(request, *args, **kwargs)

    def create(self, request, *args, **kwargs):
        _, item, _, can_write = self._check_access()
        if not can_write:
            return Response(
                {"detail": "Attachments can only be added while the quote draft is open."},
                status=status.HTTP_403_FORBIDDEN,
            )
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        attachment = serializer.save(quote_item=item)
        return Response(QuoteItemAttachmentSerializer(attachment).data, status=status.HTTP_201_CREATED)

    def destroy(self, request, *args, **kwargs):
        _, _, _, can_write = self._check_access()
        if not can_write:
            return Response(
                {"detail": "Attachments can only be removed while the quote draft is open."},
                status=status.HTTP_403_FORBIDDEN,
            )
        return super().destroy(request, *args, **kwargs)
class ShopViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Shop.objects.filter(owner=self.request.user).distinct()
...
class MachinePrintingRateViewSet(viewsets.ModelViewSet):
    ...
    def _get_machine(self):
        ...
        if not machine or not can_manage_shop(machine.shop, self.request.user):
            from rest_framework.exceptions import PermissionDenied, NotFound
            if not machine:
...
class ShopFinishingRateViewSet(ShopScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        shop = self._get_shop()
        qs = FinishingRate.objects.filter(shop=shop)
        category_slug = self.request.query_params.get("category")
        if category_slug:
            qs = qs.filter(Q(slug__icontains=category_slug) | Q(name__icontains=category_slug))
        return qs
...
class TweakedItemUpdateView(APIView):
    ...
    def patch(self, request, item_id):
        item = get_object_or_404(
            QuoteItem.objects.select_related("quote_request__shop", "product", "paper", "machine"),
            pk=item_id,
        )
        ...
        fk_fields = {"paper": Paper, "machine": Machine}
