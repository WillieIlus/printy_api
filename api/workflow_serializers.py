...
class CalculatorConfigOptionSerializer(serializers.Serializer):
...
class CalculatorConfigProductSerializer(serializers.Serializer):
    key = serializers.CharField()
    label = serializers.CharField()
    required_fields = serializers.ListField(child=serializers.CharField(), required=False)
    optional_fields = serializers.ListField(child=serializers.CharField(), required=False)
    defaults = serializers.JSONField(required=False)
    allowed_paper_categories = serializers.ListField(child=serializers.CharField(), required=False)
    allowed_cover_categories = serializers.ListField(child=serializers.CharField(), required=False)
    allowed_insert_categories = serializers.ListField(child=serializers.CharField(), required=False)
    allowed_finishings = serializers.ListField(child=serializers.CharField(), required=False)
    ...
    color_mode_options = serializers.JSONField(required=False)
...
class CalculatorConfigSerializer(serializers.Serializer):
    ...
    color_modes = CalculatorConfigOptionSerializer(many=True)
    preview_endpoint = serializers.CharField()
...
class ProductionOptionReadSerializer(serializers.ModelSerializer):
    ...
    printer_is_vat_enabled = serializers.BooleanField(source="shop.is_vat_enabled", read_only=True)
    printer_vat_rate = serializers.DecimalField(max_digits=8, decimal_places=2, source="shop.vat_rate", read_only=True)
    printer_vat_mode = serializers.CharField(source="shop.vat_mode", read_only=True)
...
class CalculatorDraftReadSerializer(serializers.ModelSerializer):
    ...
    def get_status(self, obj):
        return normalize_calculator_draft_status(
            ...
            has_pricing=bool(getattr(obj, "pricing_snapshot", None)),
        )
