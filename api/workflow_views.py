...
from accounts.services.roles import CANONICAL_PARTNER_ROLE, get_actor_role, is_client, is_shop_owner, resolve_user_roles
...
NO_MANAGEABLE_SHOP_DETAIL = "No manageable shop was found for this user. Complete shop setup before setting your rate card."


def _resolve_or_bootstrap_rate_card_shop(*, request, shop_slug: str | None = None, shop_details: dict | None = None):
    """Resolve the shop a rate card targets, bootstrapping a minimal Shop for
    shop-owner users who haven't created one yet (MVP rate card onboarding).

    Returns ``(shop, created)``. ``shop`` is ``None`` when the user is not a
    shop owner and has no manageable shop — callers respond with a clear error.
    """
    if Shop.objects.filter(owner=request.user).exists():
        return _resolve_wizard_shop(request=request, shop_slug=shop_slug), False
    if not is_shop_owner(request.user):
        return None, False
    return _create_shop_from_rate_card_draft(user=request.user, shop_details=shop_details or {}), True
...
class CalculatorConfigView(APIView):
    ...
    def get(self, request):
        ...
        try:
            return Response(CalculatorConfigSerializer(get_calculator_config()).data)
...
class ShopMvpRateCardSetupView(APIView):
    ...
    def get(self, request):
        shop, _created = _resolve_or_bootstrap_rate_card_shop(
            request=request,
            shop_slug=request.query_params.get("shop_slug"),
        )
        if shop is None:
            return Response({"detail": NO_MANAGEABLE_SHOP_DETAIL}, status=status.HTTP_404_NOT_FOUND)
    ...
    def patch(self, request):
        ...
        shop, _created = _resolve_or_bootstrap_rate_card_shop(
            request=request,
            shop_slug=request.data.get("shop_slug") or request.query_params.get("shop_slug"),
            shop_details=serializer.validated_data.get("shop_details") or {},
        )
        if shop is None:
            return Response({"detail": NO_MANAGEABLE_SHOP_DETAIL}, status=status.HTTP_404_NOT_FOUND)
...
class BuyerQuotesListView(APIView):
    """Unified 'my quotes' feed for a buyer dashboard.

    Returns the authenticated client's own calculator drafts (not yet sent)
    and already-submitted quote requests in a single list, newest first.
    Each item is tagged ``item_type`` ("draft" | "quote_request") so the
    frontend can render the two kinds together. Drafts that already produced
    a quote request are omitted — the request itself represents them.
    """
    ...
    def get(self, request):
        if not is_client(request.user):
            return Response(
                {"detail": "Only client accounts have a buyer quotes list."},
                status=status.HTTP_403_FORBIDDEN,
            )
        drafts = (
            CalculatorDraft.objects.filter(user=request.user)
            .select_related("selected_product")
            .exclude(generated_requests__isnull=False)
        )
        requests = QuoteRequest.objects.filter(
            Q(created_by=request.user) | Q(assigned_manager=request.user)
        ).select_related("source_draft", "assigned_manager")

        draft_items = [
            {
                "item_type": "draft",
                "draft": CalculatorDraftReadSerializer(draft, context={"request": request}).data,
            }
            for draft in drafts
        ]
        request_items = [
            {
                ...
                "quote_request": QuoteRequestReadSerializer(qr, context={"request": request}).data,
            }
            for qr in requests
        ]
        items = draft_items + request_items

        def _sort_key(item):
            payload = item.get("draft") or item.get("quote_request") or {}
            return payload.get("updated_at") or payload.get("created_at") or ""
