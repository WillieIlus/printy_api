...
Uses explicit schema: Product fields, Paper, Machine, PrintingRate.
...
def public_shop_products_queryset(shop):
    ...
    return public_products_queryset()
...
def get_product_starting_price(product: Product) -> dict:
    ...
    if product.pricing_mode == PricingMode.SHEET:
        errors.append("Product catalog entries are not shop-specific; use shop rate tables for pricing.")
    ...
    return {
        "price": None,
        "is_valid": False,
        "errors": ["Large-format material pricing is postponed for MVP."],
        "warnings": warnings,
        "assumptions": {},
    }
...
def compute_product_price_range_est(product: Product) -> dict:
    ...
    return {
        "can_calculate": False,
        "price_display": "Price on request",
        "pricing_mode_label": PRICING_MODE_LABELS.get(product.pricing_mode, str(product.pricing_mode or "Sheet")),
        "pricing_mode_explanation": "Product catalog entries are not shop-specific; use shop rate tables for pricing.",
        "lowest": _empty_range_payload(),
        "highest": _empty_range_payload(),
        "reason": "Shop pricing is required.",
        "missing_fields": ["shop_pricing"],
        "suggestions": [],
    }
...
def update_product_price_range(product: Product) -> dict:
    """Return the current on-demand price range without persisting it to Product."""
    return get_product_price_range(product)
