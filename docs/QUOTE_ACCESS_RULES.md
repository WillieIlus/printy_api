...
| Staff bypass is applied in `IsQuoteRequestBuyer`, `IsQuoteRequestSeller`, `IsQuoteOwner`, `IsJobCustomerOrShopOwner` |
...
| `IsQuoteOwner` | Sent quotes | `obj.shop.owner_id == user.id` |
| `IsJobCustomerOrShopOwner` | Jobs | Shop owner OR `quote.quote_request.created_by_id == user.id` |
...
| **ProductionOrderWriteSerializer** | `_get_shop_from_request` now verifies ownership when creating jobs without `quote`. |
