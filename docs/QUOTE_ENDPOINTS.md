...
**Permissions:** `IsAuthenticated`, `IsQuoteOwner`. Shop owner only. Staff can access any.
...
| POST | `/jobs/` | Create job (shop only; optionally with `quote` for create-from-quote) |
