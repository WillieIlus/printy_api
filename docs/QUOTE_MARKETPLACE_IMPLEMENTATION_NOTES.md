...
- **Quote** – shop's priced offers in response
...
**Attachments:** `QuoteAttachment` (file, name) – proofs, revised specs.
...
| Field | Type | Why it exists |
|-------|------|---------------|
| **quote** | FK | Originating accepted quote. |
...
| Migration | Purpose |
|-----------|---------|
| `quotes.0006` | Create Quote; add quote FK to QuoteItem and QuoteShareLink |
| `quotes.0007` | Data migration: create Quote from QuoteRequest, map statuses |
| `quotes.0008` | Remove legacy QuoteRequest fields; migrate QuoteShareLink |
| `quotes.0009` | Add delivery_address, delivery_location, delivery_preference, QuoteRequestAttachment, QuoteAttachment; add note, turnaround_days, revision_number to Quote |
| `production.0004` | Add quote FK; migrate status values |
...
QuoteRequest 1──* Quote

Quote 1──* QuoteItem (quote, nullable)
Quote 1──* QuoteAttachment
Quote 1──* QuoteShareLink
Quote 1──* ProductionOrder (quote, nullable)
```
...
- **API/views**: Update for new statuses; add QuoteViewSet; move price/send logic to Quote
- **QuoteSharePublicView**: Resolve via quote
- **WhatsApp formatter**: Use Quote for whatsapp_message, sent_at
