...
| Serializer | Use | Fields |
|------------|-----|--------|
| **QuoteCreateSerializer** | POST create/send | total, note, turnaround_days (quote_request from context) |
| **QuoteUpdateSerializer** | PATCH revise | note, turnaround_days, total |
| **QuoteListSerializer** | GET list | id, quote_request_id, shop, status, total, turnaround_days, revision_number, sent_at |
| **QuoteDetailSerializer** | GET detail | Full quote + items + quote_request summary |
...
| Serializer | Use | Fields |
|------------|-----|--------|
| **ProductionOrderListSerializer** | GET list | id, quote, customer, order_number, title, quantity, status, delivery_status, due_date, completed_at, delivered_at |
| **ProductionOrderSerializer** | GET detail | Full order + processes + quote_total |
...
Works with **Quote** (share links point to quote). No auth required.
