...
class PublicManagedJobTrackingView(APIView):
    ...
    def get(self, request, token):
        managed_job = get_object_or_404(
            ...
            Q(tracking_token=token) | Q(managed_reference__iexact=token),
        )
...
class ManagedJobArtworkConfirmationRequestView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        serializer.is_valid(raise_exception=True)
        try:
            managed_job = request_client_artwork_confirmation(
                managed_job=managed_job,
                actor=request.user,
                note=serializer.validated_data.get("note", ""),
            )
            ...
            return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response({"artwork_confirmation": get_artwork_confirmation_payload(managed_job)})


class ManagedJobArtworkConfirmationResponseView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        if not _can_access_managed_job(user=request.user, managed_job=managed_job, actor=actor):
            return Response({"detail": _("Not authorized.")}, status=status.HTTP_403_FORBIDDEN)
        if actor not in {OPS_ACTOR, CLIENT_ACTOR}:
            return Response({"detail": _("Not authorized.")}, status=status.HTTP_403_FORBIDDEN)
        action_name = str(request.data.get("action") or "").lower()
        if action_name not in {"approve", "reject"}:
            return Response({"detail": _("Unsupported artwork confirmation action.")}, status=status.HTTP_400_BAD_REQUEST)
        serializer = JobActionSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            managed_job = respond_to_client_artwork_confirmation(
                managed_job=managed_job,
                actor=request.user,
                approved=action_name == "approve",
                note=serializer.validated_data.get("note", ""),
            )
        except ValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
