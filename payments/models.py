...
class PaymentPhoneConsent(TimeStampedModel):
    """Audit record for saving an M-Pesa payment phone to a user profile."""

    SOURCE_QUOTE_PAYMENT = "quote_payment"
    SOURCE_MANAGED_JOB_PAYMENT = "managed_job_payment"
    SOURCE_CHOICES = [
        (SOURCE_QUOTE_PAYMENT, _("Quote payment")),
        (SOURCE_MANAGED_JOB_PAYMENT, _("Managed job payment")),
    ]

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="payment_phone_consents",
        verbose_name=_("user"),
    )
    payment = models.ForeignKey(
        Payment,
        on_delete=models.CASCADE,
        related_name="phone_consents",
        verbose_name=_("payment"),
    )
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["user", "created_at"], name="phone_consent_user_idx"),
            models.Index(fields=["payment", "created_at"], name="phone_consent_payment_idx"),
        ]

    def __str__(self):
        return f"Phone consent #{self.id or 'new'} for payment #{self.payment_id}"
