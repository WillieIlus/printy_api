...
from jobs.serializers import ManagedJobSerializer
...
class CanonicalPaymentFlowTestCase(TestCase):
    ...
    def test_successful_callback_creates_managed_job_once(self):
        ...
        self.assertEqual(
            managed_job.managed_reference,
            f"MJ-{self.quote_request.created_at:%Y%m%d}-{self.quote_request.id:04d}",
        )
    ...
    def test_old_managed_job_reference_still_serializes_safely(self):
        legacy_job = ManagedJob.objects.create(
            managed_reference="MJ-10",
        ...
        payload = ManagedJobSerializer(legacy_job).data

        self.assertEqual(payload["managed_reference"], "MJ-10")
        self.assertEqual(payload["job_reference"], "MJ-10")
        self.assertEqual(payload["quote_request_reference"], f"QR-{self.quote_request.id}")
        self.assertEqual(payload["quote_reference"], f"Q-{self.quote.id}")
    ...
    @override_settings(MPESA_ENV="sandbox", MPESA_ENVIRONMENT="test")
    def test_stk_push_without_phone_save_consent_does_not_update_profile_phone(self):
        ...
        self.assertFalse(UserProfile.objects.filter(user=self.client_user, phone="+254711111111").exists())
        self.assertFalse(PaymentPhoneConsent.objects.filter(user=self.client_user).exists())
        self.assertTrue(MpesaSTKRequest.objects.filter(phone_number="254711111111").exists())

    @override_settings(MPESA_ENV="sandbox", MPESA_ENVIRONMENT="test")
    def test_stk_push_with_phone_save_consent_updates_profile_and_audits(self):
        api_client = APIClient()
        api_client.force_authenticate(user=self.client_user)
        ...
        response = api_client.post(
            ...
            format="json",
            HTTP_USER_AGENT="Printy test client",
        )

        self.assertEqual(response.status_code, 201)
        profile = UserProfile.objects.get(user=self.client_user)
        self.assertEqual(profile.phone, "254722222222")
        consent = PaymentPhoneConsent.objects.get(user=self.client_user)
        self.assertEqual(consent.phone_number, "254722222222")
        self.assertEqual(consent.source, PaymentPhoneConsent.SOURCE_QUOTE_PAYMENT)
    ...
    def test_existing_stk_request_repairs_processing_payment_state(self):
        _quote, payment = accept_quote_for_payment(quote=self.quote, accepted_by=self.client_user)
        stale_stk = MpesaSTKRequest.objects.create(
            payment=payment,
            phone_number="254700000000",
            amount=payment.amount,
            account_reference=payment.account_reference,
            checkout_request_id="TEST-EXISTING-CR",
            merchant_request_id="TEST-EXISTING-MR",
            status=MpesaSTKRequest.STATUS_SENT,
        )

        returned = initiate_stk_push(payment=payment, phone_number="+254700000000")

        self.assertEqual(returned.id, stale_stk.id)
        payment.refresh_from_db()
        self.assertEqual(payment.status, Payment.STATUS_PROCESSING)
        self.assertEqual(payment.checkout_request_id, stale_stk.checkout_request_id)
        self.assertEqual(payment.merchant_request_id, stale_stk.merchant_request_id)

    @override_settings(MPESA_ENVIRONMENT="sandbox")
    def test_payment_stk_push_provider_failure_returns_502_and_keeps_backend_rows(self):
        api_client = APIClient()
        api_client.force_authenticate(user=self.client_user)
        ...
        with patch("payments.services.MpesaDarajaClient") as MockClient:
            ...
            mock_instance.initiate_stk_push.side_effect = ValueError("Daraja unavailable")
        ...
        self.assertEqual(response.json()["detail"], "Daraja unavailable")
        payment = Payment.objects.get(quote=self.quote)
        self.assertEqual(payment.status, Payment.STATUS_PENDING)
        stk_request = MpesaSTKRequest.objects.get(payment=payment)
        self.assertEqual(stk_request.status, MpesaSTKRequest.STATUS_FAILED)
        self.assertEqual(stk_request.response_description, "Daraja unavailable")
    ...
    def test_initiate_stk_push_handles_daraja_failure(self):
        ...
        persisted = MpesaSTKRequest.objects.get(payment=payment)
        self.assertEqual(persisted.status, MpesaSTKRequest.STATUS_FAILED)
        self.assertEqual(persisted.response_description, "Invalid phone number")
        payment.refresh_from_db()
        self.assertEqual(payment.status, Payment.STATUS_PENDING)
    ...
    def _paid_dispatched_managed_job(self):
        ...
        mark_payment_paid(payment)
        payment.refresh_from_db()
        managed_job = payment.managed_job
        assignment = ensure_job_assignment_for_paid_job(managed_job=managed_job, dispatched_by=self.broker)
        managed_job.refresh_from_db()
        return managed_job, assignment

    def test_manager_settlement_shows_expected_manager_payout_only(self):
        managed_job, _assignment = self._paid_dispatched_managed_job()
        api_client = APIClient()
        api_client.force_authenticate(self.broker)

        response = api_client.get(reverse("managed-job-settlement", kwargs={"pk": managed_job.id}))
        ...
        self.assertIn("not yet disbursed", payload["message"].lower())
    ...
    def test_shop_settlement_shows_expected_production_payout_only(self):
        managed_job, assignment = self._paid_dispatched_managed_job()
        api_client = APIClient()
        api_client.force_authenticate(self.shop_owner)

        response = api_client.get(reverse("managed-job-settlement", kwargs={"pk": managed_job.id}))

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["expected_production_payout"], str(assignment.shop_payout))
        self.assertEqual(payload["payout_status"], "pending_completion")
        self.assertFalse(payload["disbursed"])
        self.assertFalse(payload["payout_disbursed"])
        self.assertIn("not yet disbursed", payload["message"].lower())
        self.assertNotIn("released", payload["message"].lower())
        self.assertNotIn("broker_payout", payload)
        self.assertNotIn("expected_manager_payout", payload)
        self.assertNotIn("client_total", payload)
        self.assertNotIn("printy_fee", payload)

    def test_client_settlement_hides_internal_payout_economics(self):
        managed_job, _assignment = self._paid_dispatched_managed_job()
        api_client = APIClient()
        ...
        forbidden = {
            "broker_payout",
            "shop_payout",
            "expected_manager_payout",
            "expected_production_payout",
            "production_cost",
            "printy_fee",
            "client_total",
        }
    ...
    def test_wrong_roles_cannot_access_settlement(self):
        managed_job, _assignment = self._paid_dispatched_managed_job()
        other_manager = User.objects.create_user(email="other-manager@example.com", password="pass", role=User.Role.PARTNER)
        other_shop_owner = User.objects.create_user(email="other-shop@example.com", password="pass", role=User.Role.PRODUCTION)
        Shop.objects.create(name="Other Shop", owner=other_shop_owner, is_active=True)
        api_client = APIClient()

        api_client.force_authenticate(other_manager)
        manager_response = api_client.get(reverse("managed-job-settlement", kwargs={"pk": managed_job.id}))
        self.assertEqual(manager_response.status_code, 403)

        api_client.force_authenticate(other_shop_owner)
        shop_response = api_client.get(reverse("managed-job-settlement", kwargs={"pk": managed_job.id}))
        self.assertEqual(shop_response.status_code, 403)

    def test_unpaid_settlement_is_not_payable_or_paid(self):
        self.quote_request.assigned_manager = self.broker
        self.quote_request.save(update_fields=["assigned_manager", "updated_at"])
        _quote, payment = accept_quote_for_payment(quote=self.quote, accepted_by=self.client_user)
        managed_job = ManagedJob.objects.create(
            title="Accepted unpaid managed job",
            source_quote_request=self.quote_request,
            source_quote=self.quote,
            client=self.client_user,
            broker=self.broker,
            ...
            client_total=self.split.client_total,
        )
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["payout_status"], "pending_payment")
        self.assertFalse(payload["disbursed"])
        self.assertNotEqual(payload["payout_status"], "paid")

    def test_released_payment_status_does_not_claim_disbursement_without_payout_record(self):
        managed_job, _assignment = self._paid_dispatched_managed_job()
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["payout_status"], "manual_payout_pending")
        self.assertEqual(payload["payout_status_label"], "Manual payout pending")
        self.assertFalse(payload["disbursed"])
        self.assertFalse(payload["payout_disbursed"])
        self.assertNotEqual(payload["payout_status"], "paid")
    ...
    def test_normalize_mpesa_phone_accepts_common_kenyan_formats(self):
        self.assertEqual(normalize_mpesa_phone("+254 700 000 000"), "254700000000")
        self.assertEqual(normalize_mpesa_phone("0700 000 000"), "254700000000")
        self.assertEqual(normalize_mpesa_phone("700000000"), "254700000000")
        self.assertEqual(normalize_mpesa_phone("0111-222-333"), "254111222333")

    def test_normalize_mpesa_phone_rejects_invalid_numbers(self):
        with self.assertRaises(ValidationError):
            normalize_mpesa_phone("12345")
        with self.assertRaises(ValidationError):
            normalize_mpesa_phone("+255700000000")

    @override_settings(MPESA_ENV="sandbox", MPESA_ENVIRONMENT="test")
    def test_stk_push_rejects_invalid_phone_without_creating_payment(self):
        api_client = APIClient()
        api_client.force_authenticate(user=self.client_user)
        ...
        self.assertIn("valid Kenyan M-Pesa phone number", response.json()["detail"])
        self.assertFalse(Payment.objects.filter(quote=self.quote).exists())

    @override_settings(MPESA_ENV="sandbox", MPESA_ENVIRONMENT="test")
    def test_stk_push_normalizes_local_phone_before_persisting(self):
        api_client = APIClient()
        api_client.force_authenticate(user=self.client_user)
        ...
        self.assertEqual(stk_request.phone_number, "254700000000")
        self.assertEqual(PaymentPhoneConsent.objects.get(user=self.client_user).phone_number, "254700000000")
