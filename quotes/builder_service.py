...
from quotes.models import QuoteItem, QuoteItemAttachment, QuoteItemFinishing, QuoteRequest, QuoteRequestMessage
...
def _clone_quote_item_to_request(*, source_item: QuoteItem, quote_request: QuoteRequest) -> QuoteItem:
    ...
    QuoteItemAttachment.objects.bulk_create(
        [
            QuoteItemAttachment(
                quote_item=cloned_item,
            ...
            for attachment in source_item.attachments.all()
            if attachment.file
        ]
    )
    return cloned_item
...
def send_quote_request_item_to_shop(*, draft: QuoteRequest, item: QuoteItem, user) -> QuoteRequest:
    ...
    with transaction.atomic():
        single_request = QuoteRequest.objects.create(
            ...
            calculator_draft_file=draft.calculator_draft_file,
