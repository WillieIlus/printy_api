...
from quotes.models import EmailOutbox, QuoteRequestMessage
...
def _build_email_payload(message):
    ...
    if message.message_type == QuoteRequestMessage.MessageType.QUOTE_REQUEST_CREATED and message.recipient_role == QuoteRequestMessage.RecipientRole.SHOP_OWNER:
        common["cta_url"] = f"{_frontend_base_url()}/app/printer"
        common.update(
            {
    ...
    if message.message_type == QuoteRequestMessage.MessageType.QUOTE_RESPONSE_SENT and message.recipient_role == QuoteRequestMessage.RecipientRole.CLIENT and quote:
        snapshot = quote.response_snapshot or {}
        common["cta_url"] = f"{_frontend_base_url()}/app/buyer?tab=quote"
        common.update(
            {
                "headline": message.subject,
...
def _prepared_email_payload_for_message(message):
    ...
    return {
        ...
        "body": render_to_string(payload["text_template"], payload["context"]),
...
def _send_prepared_email(*, recipient_email: str, subject: str, body: str, html_body: str = ""):
    email = EmailMultiAlternatives(
        subject=subject or "Printy quote update",
        body=body or subject or "Printy quote update",
...
def _create_email_outbox_entry(message_id: int):
    message = QuoteRequestMessage.objects.select_related("recipient", "quote_request", "quote").get(pk=message_id)
    if not message.recipient_email:
        return None
    prepared = _prepared_email_payload_for_message(message)
    outbox_entry = EmailOutbox.objects.create(
        recipient=message.recipient,
        recipient_email=message.recipient_email,
        subject=prepared["subject"],
        body=prepared["body"],
        html_body=prepared["html_body"],
        text_template=prepared["text_template"],
        html_template=prepared["html_template"],
        context=prepared["context"],
        quote_request=message.quote_request,
        quote=message.quote,
        message=message,
    )
    _auto_send_email_outbox_entry(outbox_entry)
    return outbox_entry


def _send_email_for_message(message):
    transaction.on_commit(lambda: _create_email_outbox_entry(message.pk))


def send_email_outbox_entry(outbox_entry: EmailOutbox):
    _send_prepared_email(
        recipient_email=outbox_entry.recipient_email,
        subject=outbox_entry.subject,
        body=outbox_entry.body,
        html_body=outbox_entry.html_body,
    )


def _mark_email_outbox_sent(outbox_entry: EmailOutbox):
    now = timezone.now()
    outbox_entry.status = EmailOutbox.Status.SENT
    outbox_entry.sent_at = now
    outbox_entry.last_error = ""
    outbox_entry.attempts_count += 1
    outbox_entry.save(update_fields=["status", "sent_at", "last_error", "attempts_count", "updated_at"])
    if outbox_entry.message_id:
        QuoteRequestMessage.objects.filter(pk=outbox_entry.message_id).update(
            email_sent=True,
            email_status=QuoteRequestMessage.EmailStatus.SENT,
            email_error="",
            updated_at=now,
        )


def _mark_email_outbox_failed(outbox_entry: EmailOutbox, exc: Exception):
    now = timezone.now()
    error_text = _safe_email_error(exc)
    next_attempts = outbox_entry.attempts_count + 1
    failed_permanently = next_attempts >= EmailOutbox.MAX_ATTEMPTS
    outbox_entry.attempts_count = next_attempts
    outbox_entry.status = (
        EmailOutbox.Status.PERMANENTLY_FAILED
        if failed_permanently
        else EmailOutbox.Status.FAILED
    )
    outbox_entry.last_error = error_text
    outbox_entry.save(update_fields=["attempts_count", "status", "last_error", "updated_at"])
    if outbox_entry.message_id:
        QuoteRequestMessage.objects.filter(pk=outbox_entry.message_id).update(
            email_sent=False,
...
def process_email_outbox_entry(outbox_entry: EmailOutbox):
    try:
        send_email_outbox_entry(outbox_entry)
    except Exception as exc:
        return "permanently_failed" if _mark_email_outbox_failed(outbox_entry, exc) else "failed"
    _mark_email_outbox_sent(outbox_entry)
    return "sent"
...
def _auto_send_email_outbox_entry(outbox_entry: EmailOutbox):
    ...
    if result != "sent":
        logger.warning(
            "Quote message email immediate send failed outbox=%s status=%s error=%s",
            outbox_entry.id,
            result,
            outbox_entry.last_error,
        )
    ...
    *,
    ...
    if send_email_copy and message.recipient_email:
        try:
            ...
            error_text = _safe_email_error(exc)
            logger.warning(
                "Quote message email enqueue failed for quote_request=%s recipient=%s type=%s: %s",
                quote_request.id,
            ...
            return message
        message.email_status = QuoteRequestMessage.EmailStatus.NOT_SENT
        message.email_error = ""
        message.email_sent = False
        message.save(update_fields=["email_status", "email_error", "email_sent", "updated_at"])
    return message
