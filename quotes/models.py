...
class QuoteRequest(TimeStampedModel):
    ...
    expected_manager_response_by = models.DateTimeField(null=True, blank=True)
    manager_responded_at = models.DateTimeField(null=True, blank=True)
    manager_response_note = models.CharField(
        max_length=255,
        blank=True,
        default="",
        verbose_name=_("manager response note"),
        help_text=_("Internal manager response note for intake requests."),
    )
    manager_response_state = models.CharField(max_length=40, blank=True, default="pending")
    manager_viewed_at = models.DateTimeField(null=True, blank=True)
...
class EmailOutbox(TimeStampedModel):
    ...
    body = models.TextField(blank=True, default="", verbose_name=_("text body"))
    html_body = models.TextField(blank=True, default="", verbose_name=_("html body"))
    text_template = models.CharField(max_length=255, blank=True, default="", verbose_name=_("text template"))
    html_template = models.CharField(max_length=255, blank=True, default="", verbose_name=_("html template"))
    context = models.JSONField(default=dict, blank=True, verbose_name=_("template context"))
    quote_request = models.ForeignKey(
        QuoteRequest,
        on_delete=models.SET_NULL,
        null=True,
    ...
    attempts_count = models.PositiveSmallIntegerField(default=0, verbose_name=_("attempts count"))
    last_error = models.CharField(max_length=255, blank=True, default="", verbose_name=_("last error"))
    sent_at = models.DateTimeField(null=True, blank=True, verbose_name=_("sent at"))

    class Meta:
        ordering = ["created_at", "id"]
        verbose_name = _("email outbox entry")
        verbose_name_plural = _("email outbox entries")
        indexes = [
            models.Index(fields=["status", "attempts_count", "created_at"], name="email_outbox_ready_idx"),
            models.Index(fields=["message"], name="email_outbox_message_idx"),
        ]
