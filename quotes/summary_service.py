...
from .models import CalculatorDraftFile, QuoteItem, QuoteRequest, Quote
...
def format_quote_summary(
    ...
    *,
    ...
    Args:
        quote: The shop quote (sent or revised).
    ...
    items = list(
        quote.items.select_related("product", "paper", "material").prefetch_related(
            "finishings__finishing_rate"
        ).all()
    )
    if not items:
    ...
    if quote.turnaround_hours is not None:
        turnaround = f"{quote.turnaround_hours} working hour(s)"
    elif quote.turnaround_days is not None:
        d = quote.turnaround_days
        turnaround = f"{d} business day(s)" if d == 1 else f"{d} business days"
    lines.append(f"Turnaround: {turnaround}")
    if quote.human_ready_text:
        lines.append(quote.human_ready_text)
    lines.append("")

    if quote.note:
        lines.append(f"Note: {quote.note[:150]}" + ("..." if len(quote.note) > 150 else ""))
        lines.append("")
...
def get_quote_request_summary_text(quote_request: QuoteRequest) -> str:
    ...
    """
    latest = quote_request.get_latest_quote()
    if latest and latest.total is not None:
        return format_quote_summary(
            latest,
...
def get_quote_summary_text(quote: Quote, share_url: Optional[str] = None) -> str:
    """
    Convenience: summary for API/serializer with optional share URL.
    """
    return format_quote_summary(
        quote,
        company_name=quote.shop.name if quote.shop_id else "",
        company_phone=quote.shop.phone_number or "" if quote.shop_id else "",
        share_url=share_url,
    )


def get_calculator_draft_file_summary_text(draft_file: CalculatorDraftFile) -> str:
...
def get_calculator_draft_file_summary_payload(draft_file: CalculatorDraftFile) -> dict:
    """Structured WhatsApp preview payload for grouped quote files."""
    payload = build_dashboard_quote_file_payload(draft_file)
    return {
        "message": get_calculator_draft_file_summary_text(draft_file),
