...
def format_quote_for_whatsapp(
    ...
    *,
    ...
    # Total lives on Quote in the current model, but callers still pass QuoteRequest.
    total = getattr(quote, "total", None)
    if total is None and hasattr(quote, "get_latest_quote"):
        latest_quote = quote.get_latest_quote()
        if latest_quote is not None:
            total = latest_quote.total
