...
def normalize_pages(total_pages: int | None, binding_type: str = "saddle_stitch") -> int:
    safe_total = max(int(total_pages or 0), 0)
    modulus = 2 if binding_type == "perfect_bind" else 4
    remainder = safe_total % modulus
    return safe_total if remainder == 0 else safe_total + (modulus - remainder)


def build_page_plan(total_pages: int | None, binding_type: str, *, cover_duplex_requested: bool = False) -> dict[str, Any]:
    raw_pages = max(int(total_pages or 0), 0)
    normalized_pages = normalize_pages(raw_pages, binding_type)
    blank_pages_added = max(normalized_pages - raw_pages, 0)
    blank_positions: list[int] = []
    cover_print_mode = "duplex" if cover_duplex_requested else "simplex"

    if binding_type == "perfect_bind":
        if blank_pages_added == 1:
            blank_positions = [normalized_pages - 1]
        return {
            "raw_pages": raw_pages,
    ...
    if blank_pages_added == 1:
        blank_positions = [normalized_pages - 1]
        cover_print_mode = "duplex"
    elif blank_pages_added == 2:
        blank_positions = [2, normalized_pages - 1]
        cover_print_mode = "simplex"
    elif blank_pages_added == 3 and cover_duplex_requested:
        blank_positions = [normalized_pages - 3, normalized_pages - 2, normalized_pages - 1]
        cover_print_mode = "duplex"
    elif blank_pages_added == 3:
        blank_positions = [2, normalized_pages - 2, normalized_pages - 1]
        cover_print_mode = "simplex"
...
def split_cover_inserts(total_pages: int | None, binding_type: str = "saddle_stitch") -> tuple[int, int, int]:
    normalized_pages = normalize_pages(total_pages, binding_type)
...
def calculate_booklet_pricing(
    *,
    ...
    page_plan = build_page_plan(raw_pages, binding_type, cover_duplex_requested=cover_sides == "DUPLEX")
    normalized_pages = int(page_plan["normalized_pages"])
    blank_pages_added = int(page_plan["blank_pages_added"])
    blank_positions = list(page_plan["blank_positions"])
    cover_print_mode = str(page_plan["cover_print_mode"])
    effective_cover_sides = "SIMPLEX" if cover_print_mode == "simplex" else cover_sides
    normalized_pages, cover_pages, insert_pages = split_cover_inserts(raw_pages, binding_type)
    if blank_pages_added:
        warnings.append("Booklet pages include production blank pages.")
        assumptions.append("Booklet blank positions are calculated by the backend.")
    if binding_type == "perfect_bind":
        assumptions.append("Perfect-bound booklets are normalized to an even page count.")
    elif binding_type == "wire_o":
        assumptions.append("Wire-O booklets are quoted using folded booklet spread assumptions normalized to multiples of 4.")
    ...
    cover_machine, cover_print_rate_obj, cover_print_rate = _resolve_printing_path(
        shop, cover_paper, cover_color_mode, effective_cover_sides
    )
    ...
    nested_breakdown = {
        ...
        "cover": {
            ...
            "print_sides": effective_cover_sides,
    ...
    if blank_pages_added:
        blank_pages_text = ", ".join(str(page) for page in blank_positions)
        client_warning_text = f"{blank_pages_added} blank page(s) will be added for production at page(s) {blank_pages_text}."
        manager_production_note = f"Booklet requires {blank_pages_added} backend-calculated blank page(s) at page(s) {blank_pages_text}; cover print mode: {cover_print_mode}."
        shop_production_note = f"Produce {normalized_pages} page booklet from {raw_pages} supplied pages. Blank page positions: {blank_pages_text}. Cover print mode: {cover_print_mode}."
    else:
        client_warning_text = ""
        manager_production_note = f"Booklet page count is production-ready at {normalized_pages} pages; no blank pages added."
        shop_production_note = f"Produce {normalized_pages} page booklet; no blank pages added. Cover print mode: {cover_print_mode}."

    nested_breakdown["booklet"]["client_warning_text"] = client_warning_text
    nested_breakdown["booklet"]["manager_production_note"] = manager_production_note
    nested_breakdown["booklet"]["shop_production_note"] = shop_production_note
    ...
    return {
        ...
        "blank_positions": blank_positions,
        "cover_print_mode": cover_print_mode,
        "client_warning_text": client_warning_text,
        "manager_production_note": manager_production_note,
        "shop_production_note": shop_production_note,
        "cover_pages": cover_pages,
        "insert_pages": insert_pages,
        "cover_sheets": cover_sheets,
        "insert_sheets": insert_sheets,
