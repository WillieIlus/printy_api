...
def _strip_vat_lines(values: list[Any]) -> list[Any]:
    return [
        value
        for value in _as_list(values)
        if not (isinstance(value, str) and value.strip().startswith("VAT:"))
    ]
...
def _sanitize_public_preview(preview: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    projected = {key: source.get(key) for key in allowed_keys if source.get(key) not in (None, "", [], {})}
    if "warnings" in projected:
        projected["warnings"] = _strip_vat_lines(projected["warnings"])
    if "explanations" in projected:
        projected["explanations"] = _strip_vat_lines(projected["explanations"])
    return projected
...
def _extract_production_preview(matches: list[dict[str, Any]], product_type: str) -> dict[str, Any] | None:
    ...
    warnings = _strip_vat_lines(preview_data.get("explanations") or preview_data.get("warnings") or [])
