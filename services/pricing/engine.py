...
_VAT_PAYLOAD_KEYS = ("vat", "vat_amount", "vat_mode")
_VAT_LINE_ITEM_CODES = {"vat"}
_VAT_EXPLANATION_PREFIX = "VAT:"


def _is_vat_explanation(text: Any) -> bool:
    return isinstance(text, str) and text.strip().startswith(_VAT_EXPLANATION_PREFIX)


def strip_vat_detail(payload: dict) -> dict:
    """Return a deep copy of an engine payload with standalone VAT detail removed.

    The VAT-adjusted ``grand_total`` is left untouched so the final price remains
    VAT-inclusive.  Only the *separate* VAT keys (amount, rate, mode), the
    dedicated VAT line-item and the "VAT: ..." explanation lines are stripped.
    """
    result = deepcopy(payload)
    for key in _VAT_PAYLOAD_KEYS:
        result.pop(key, None)
    ...
    if isinstance(calc, dict):
        ...
        if isinstance(line_items, list):
            calc["line_items"] = [
                ...
                if not (isinstance(line, dict) and line.get("code") in _VAT_LINE_ITEM_CODES)
            ]
    for key in ("explanations", "explanation_lines"):
        values = result.get(key)
        if isinstance(values, list):
            result[key] = [text for text in values if not _is_vat_explanation(text)]
    return result
...
class PricingEngineResult:
    ...
    def to_dict(self, *, expose_vat_detail: bool = True) -> dict:
        payload = asdict(self)
        payload["calculation_result"] = build_contract_from_engine_payload(payload)
        if not expose_vat_detail:
            payload = strip_vat_detail(payload)
        return payload
