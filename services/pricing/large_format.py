...
def _build_roll_layout(*, product_subtype: str, width_mm: int, height_mm: int, quantity: int, material):
    ...
    if not media:
        return None, ["No production roll width is configured for this material; pricing cannot be calculated."], []
...
def calculate_large_format_preview(
    *,
    ...
    if layout_result is None:
        reason = "No production roll width is configured for this material. Add a roll width before pricing large-format work."
        warnings = list(layout_warnings) or [reason]
        assumptions = [
            f"Subtype pricing uses the {str(subtype['label']).lower()} profile.",
            "Large-format pricing requires backend roll-width configuration.",
            *layout_assumptions,
        ]
        calculation_result = build_calculation_result(
            quote_type="large_format",
            pricing_mode=PricingMode.LARGE_FORMAT,
            billing_type="per_roll",
        ...
        return {
            ...
            "breakdown": {
                ...
                "roll_usage": None,
                "tiling": {"required": False, "panels": []},
            },
            "client_warning_text": reason,
            "manager_production_note": reason,
            "shop_production_note": reason,
