...
raise unittest.SkipTest("Legacy pre-reset calculator tests target removed Product.shop relationships.")
...
class QuoteCalculatorTestCase(TestCase):
    ...
    def test_duplex_surcharge_applied_on_top_of_double_price_override(self):
        """
        Regression: when double_price override is set, the duplex surcharge must still be
        added for qualifying papers. double_price is the base print cost for both sides;
        surcharge = paper + printing_double + surcharge (formula from spec).
        """
        PrintingRate.objects.all().delete()
        self.paper.selling_price = Decimal("7.00")
        self.paper.gsm = 150
        self.paper.save(update_fields=["selling_price", "gsm"])
        PrintingRate.objects.create(
            machine=self.machine,
            ...
            color_mode="COLOR",
            single_price=Decimal("15.00"),
            double_price=Decimal("30.00"),  # base duplex override (both sides, no surcharge)
            duplex_surcharge=Decimal("5.00"),
            duplex_surcharge_enabled=True,
            duplex_surcharge_min_gsm=150,
            is_active=True,
        ...
        # double = double_price_override(30) + surcharge(5) = 35; total_per_sheet = paper(7) + 35 = 42
        self.assertEqual(result.breakdown["printing"]["duplex_surcharge"], "5.00")
        self.assertTrue(result.breakdown["printing"]["duplex_surcharge_applied"])
        self.assertEqual(result.breakdown["printing"]["total_per_sheet"], "35.00")
        self.assertEqual(result.totals["total_per_sheet"], "42.00")

    def test_duplex_override_without_surcharge_unaffected(self):
        """
        Regression guard: double_price override with surcharge disabled must still give
        total = double_price only (no surcharge added).
        """
        PrintingRate.objects.all().delete()
        self.paper.selling_price = Decimal("24.00")
        self.paper.gsm = 300
        self.paper.save(update_fields=["selling_price", "gsm"])
        PrintingRate.objects.create(
            machine=self.machine,
            ...
            duplex_surcharge=Decimal("5.00"),
            duplex_surcharge_enabled=False,  # disabled — surcharge must NOT be added
            is_active=True,
        )
