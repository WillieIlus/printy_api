...
class Step9BackendWorkflowTestCase(TestCase):
    ...
    def test_client_draft_send_cannot_route_directly_to_shops(self):
        ...
        self.assertRegex(requests[0].request_reference, r"^QR-\d{8}-\d{4}$")
    ...
    def test_full_guest_to_client_to_production_tracking_workflow(self):
        ...
        self.assertRegex(quote.quote_reference, r"^Q-\d{8}-\d{4}$")
        ...
        self.assertEqual(
            ...
            f"MJ-{quote_request.created_at:%Y%m%d}-{quote_request.id:04d}",
        )
